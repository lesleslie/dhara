...
# Alias old Durus 4.x class names to new druva classes
...
class _DurusCompatModule:
    ...
    def __getattr__(self, name):
        if name not in self._modules:
            ...
            if name == "persistent":
                from druva.core import persistent

                self._modules[name] = persistent
            elif name == "persistent_dict":
                from druva.collections import dict as persistent_dict

                self._modules[name] = persistent_dict
            elif name == "persistent_list":
                from druva.collections import list as persistent_list

                self._modules[name] = persistent_list
            elif name == "persistent_set":
                from druva.collections import set as persistent_set
