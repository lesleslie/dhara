...
from druva.core.config import StorageConfig
...
class LiteMode(OperationalMode):
    ...
    - Local filesystem storage in ~/.local/share/druva/
    ...
    # Default paths
    DEFAULT_STORAGE_PATH = Path.home() / ".local" / "share" / "druva" / "lite.druva"
        ...
        try:
            ...
            if not self._is_port_available(self.DEFAULT_HOST, self.DEFAULT_PORT):
                logger.warning(
                    ...
                    f"If startup fails, try: druva start --mode=lite --port=8684"
                )
...
║                                                                   ║
║   ✓ Zero configuration required                                  ║
║   ✓ Local storage in ~/.local/share/druva/                      ║
║   ✓ Host: 127.0.0.1:8683                                         ║
║   ✓ Ideal for development and testing                            ║
║                                                                   ║
║   Quick start:                                                   ║
...
"""
    ...
    def get_info(self) -> dict[str, Any]:
        ...
        info.update(
            {
                "startup_command": "druva start --mode=lite",
