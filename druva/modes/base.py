...
class LiteMode(OperationalMode):
    ...
    def get_default_storage_path(self) -> Path:
        return Path.home() / ".local" / "share" / "druva" / "lite.druva"
...
class StandardMode(OperationalMode):
    ...
    def get_default_storage_path(self) -> Path:
        return Path("/data/druva/production.druva")
...
def get_mode(settings: DruvaSettings | None = None) -> OperationalMode:
    ...
    env_mode = os.getenv("DRUVA_MODE", "").lower().strip()
    ...
    if settings:
        ...
        lite_path = Path.home() / ".local" / "share" / "druva" / "lite.druva"
