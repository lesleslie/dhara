...
from druva.core.config import DruvaSettings, StorageConfig
from druva.modes.base import ModeValidationError, OperationalMode
...
class StandardMode(OperationalMode):
    ...
    # Default paths
    DEFAULT_STORAGE_PATH = Path("/data/druva/production.druva")
    DEFAULT_CONFIG_PATH = (
    ...
    def validate_environment(self) -> bool:
        ...
        if self.settings is None:
            try:
                self.settings = DruvaSettings.load("druva")
...
║                                                                   ║
║   Configuration:                                                 ║
║     $ druva start --mode=standard --config=settings/production  ║
║                                                                   ║
╚═══════════════════════════════════════════════════════════════════╝
"""
    ...
    def get_info(self) -> dict[str, Any]:
        ...
        info.update(
            {
                "startup_command": "druva start --mode=standard",
    ...
    def _validate_file_storage(self) -> None:
        ...
        if not storage_path:
            raise ModeValidationError(
                ...
                mode_name=self.name,
                details={"fix": "Set DRUVA_STORAGE_PATH or configure in YAML"},
            )
        ...
        if self.settings and hasattr(self.settings, "s3_bucket"):
            if not self.settings.s3_bucket:
                raise ModeValidationError(
                    ...
                    mode_name=self.name,
                    details={"fix": "Set DRUVA_S3_BUCKET or configure in YAML"},
                )
        else:
        ...
        if self.settings and hasattr(self.settings, "gcs_bucket"):
            if not self.settings.gcs_bucket:
                raise ModeValidationError(
                    ...
                    mode_name=self.name,
                    details={"fix": "Set DRUVA_GCS_BUCKET or configure in YAML"},
                )
        else:
        ...
        if self.settings and hasattr(self.settings, "azure_container"):
            if not self.settings.azure_container:
                raise ModeValidationError(
                    ...
                    mode_name=self.name,
                    details={"fix": "Set DRUVA_AZURE_CONTAINER or configure in YAML"},
                )
        else:
    ...
    def _validate_network_access(self) -> None:
        ...
        if not self._is_port_available(host, port):
            logger.warning(
                ...
                f"If startup fails, try: druva start --mode=standard --port=8684"
            )
    ...
    def _configure_file_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        config.backend = "file"
        if not config.path or config.path == Path("/data/druva/druva.druva"):
    ...
    def _configure_sqlite_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        if not config.path or config.path == Path("/data/druva/druva.druva"):
            config.path = Path("/data/druva/production.db")
        return config
    ...
    def _configure_s3_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        if not config.path or config.path == Path("/data/druva/druva.druva"):
            config.path = Path("s3://druva-production/druva.druva")
        return config
    ...
    def _configure_gcs_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        config.backend = "gcs"
        if not config.path or config.path == Path("/data/druva/druva.druva"):
            config.path = Path("gs://druva-production/druva.druva")
        return config
    ...
    def _configure_azure_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        if not config.path or config.path == Path("/data/druva/druva.druva"):
            config.path = Path("azure://druva-production/druva.druva")
        return config
