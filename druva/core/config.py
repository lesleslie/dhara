"""Druva MCP Server configuration using mcp-common patterns.
...
class StorageConfig(BaseModel):
    """Druva storage configuration."""

    path: Path = Field(default=Path("/data/druva.druva"))
...
class DruvaSettings(MCPServerSettings):
    """Druva MCP Server settings extending MCPServerSettings.

    Configuration loading order (later overrides earlier):
    1. Default values (below)
    2. settings/druva.yaml (committed) OR settings/{mode}.yaml
    3. settings/local.yaml (gitignored, for development)
    4. Environment variables: DRUVA_{FIELD}

    Example YAML (settings/druva.yaml):
        mode: standard
        storage:
    ...
    Example env vars:
        ...
        export DRUVA_STORAGE_PATH=/custom/path
        export DRUVA_STORAGE_READ_ONLY=false
    """
    ...
    @classmethod
    def load(cls, config_name: str = "druva") -> DruvaSettings:
        """Load settings with mode-aware configuration.

        Detects mode from environment and loads appropriate config file:
        - DRUVA_MODE=lite → settings/lite.yaml
        - DRUVA_MODE=standard → settings/standard.yaml
        - No mode set → settings/druva.yaml

        Args:
            config_name: Base config name (default: "druva")

        Returns:
            Loaded DruvaSettings instance
        """
        # Detect mode from environment
        mode = os.getenv("DRUVA_MODE", "").lower().strip()
    ...
    def get_mode_config_path(self) -> Path:
        ...
        if self.mode == "lite":
            ...
            return Path(__file__).parent.parent.parent / "settings" / "druva.yaml"
    ...
    def health_snapshot_path(self) -> Path:
        ...
        """
        snapshot_name = f"{self.mode}_druva_health.json"
