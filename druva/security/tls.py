"""
TLS/SSL support for druva network storage.
...
def get_env_tls_config() -> TLSConfig | None:
    ...
    Environment variables:
        DRUVA_TLS_CERTFILE: Path to server certificate
        DRUVA_TLS_KEYFILE: Path to server private key
        DRUVA_TLS_CAFILE: Path to CA certificate
        DRUVA_TLS_CAPATH: Path to CA directory
        DRUVA_TLS_CLIENT_CERTFILE: Path to client certificate (mutual TLS)
        DRUVA_TLS_CLIENT_KEYFILE: Path to client private key (mutual TLS)
        DRUVA_TLS_VERIFY_MODE: Verification mode (none, optional, required)
        DRUVA_TLS_VERSION: Minimum TLS version (1.2, 1.3)
        DRUVA_TLS_CHECK_HOSTNAME: Enable hostname verification (true/false)

    Returns:
        TLSConfig if any TLS environment variables are set, None otherwise
    """
    certfile = os.getenv("DRUVA_TLS_CERTFILE")
    keyfile = os.getenv("DRUVA_TLS_KEYFILE")
    cafile = os.getenv("DRUVA_TLS_CAFILE")
    capath = os.getenv("DRUVA_TLS_CAPATH")
    client_certfile = os.getenv("DRUVA_TLS_CLIENT_CERTFILE")
    client_keyfile = os.getenv("DRUVA_TLS_CLIENT_KEYFILE")
    ...
    # Parse verify mode
    verify_mode_str = os.getenv("DRUVA_TLS_VERIFY_MODE", "required").lower()
    verify_mode_map = {
        "none": ssl.CERT_NONE,
    ...
    # Parse TLS version
    tls_version_str = os.getenv("DRUVA_TLS_VERSION", "1.3")
    tls_version_map = {
    ...
    check_hostname = os.getenv("DRUVA_TLS_CHECK_HOSTNAME", "true").lower() == "true"
