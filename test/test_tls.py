"""
Tests for TLS/SSL support in dhara.
...
class TestEnvTLSConfig:
    ...
    def test_no_env_vars_returns_none(self):
        ...
        for key in list(os.environ.keys()):
            if key.startswith("DHARA_TLS_") or key.startswith("DRUVA_TLS_"):
                del os.environ[key]
    ...
    def test_env_vars_create_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            os.environ["DHARA_TLS_CERTFILE"] = str(certfile)
            os.environ["DHARA_TLS_KEYFILE"] = str(keyfile)
            ...
            try:
                ...
                # Clean up
                del os.environ["DHARA_TLS_CERTFILE"]
                del os.environ["DHARA_TLS_KEYFILE"]
    ...
    def test_verify_mode_from_env(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for mode, expected in [
                ...
                os.environ["DHARA_TLS_VERIFY_MODE"] = mode
                try:
                    config = get_env_tls_config()
                    assert config.verify_mode == expected
                finally:
                    if "DHARA_TLS_VERIFY_MODE" in os.environ:
                        del os.environ["DHARA_TLS_VERIFY_MODE"]
    ...
    def test_wrap_server_socket(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                pytest.skip(f"Sandbox does not allow local socket bind: {exc}")
            sock.listen(1)
