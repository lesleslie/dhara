...
import dhruva.storage.base as dhruva_storage
...
class Connection(ConnectionBase):
    ...
    def __init__(self, storage, cache_size=100000, root_class=None):
        ...
        assert isinstance(storage, dhruva_storage.Storage)
...
class Cache:
    def __init__(self, size):
        ...
        self.set_size(size)
        # OrderedDict for true LRU tracking (O(k) vs O(n log n) heap)
    ...
    def get(self, oid):
        ...
        if obj is not None:
            ...
            self._lru[oid] = None  # Value doesn't matter, we just want order
    ...
    def shrink(self, connection):
        ...
        Uses OrderedDict-based LRU eviction for O(k) performance where k is
        the number of objects to evict. This is significantly faster than the
        previous O(n log n) heap-based approach.

        Objects accessed in the current transaction are never ghosted.
        """
        ...
        if current <= target:
            ...
            log(
                10,
                "[%s] cache size %s recent %s",
                getpid(),
                current,
                len(self.recent_objects),
            )
            return

        start_time = time()
        transaction_serial = connection.get_transaction_serial()
        num_ghosted = 0
        removed = 0

        # Evict least recently used objects that aren't in current transaction
        # OrderedDict maintains insertion order, so we iterate from oldest to newest
        to_remove = current - target
        for oid in list(self._lru.keys()):
            if removed >= to_remove or len(self.objects) <= target:
                break

            obj = self.objects.get(oid)
            if obj is None:
                # Object was garbage collected, clean up LRU
                del self._lru[oid]
                continue

            # Skip objects accessed in current transaction
        ...
        log(
            10,
            "[%s] shrink O(k) %fs removed %s ghosted %s size %s recent %s",
            getpid(),
            time() - start_time,
            removed,
            num_ghosted,
