...
This mode provides complete Dhruva capabilities:
...
╔═══════════════════════════════════════════════════════════════════╗
║                                                                   ║
║   🦀 Dhruva Standard Mode - Production Ready                     ║
║                                                                   ║
...
║                                                                   ║
║   Configuration:                                                 ║
║     $ dhruva start --mode=standard --config=settings/production  ║
║                                                                   ║
╚═══════════════════════════════════════════════════════════════════╝
"""
    ...
    def _validate_network_access(self) -> None:
        ...
        if not self._is_port_available(host, port):
            logger.warning(
                ...
                f"If startup fails, try: dhruva start --mode=standard --port=8684"
            )
    ...
    def _configure_file_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        config.backend = "file"
        if not config.path or config.path == Path("/data/dhruva/dhruva.dhruva"):
    ...
    def _configure_sqlite_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        if not config.path or config.path == Path("/data/dhruva/dhruva.dhruva"):
            config.path = Path("/data/dhruva/production.db")
        return config
    ...
    def _configure_s3_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        if not config.path or config.path == Path("/data/dhruva/dhruva.dhruva"):
            config.path = Path("s3://dhruva-production/dhruva.dhruva")
        return config
    ...
    def _configure_gcs_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        config.backend = "gcs"
        if not config.path or config.path == Path("/data/dhruva/dhruva.dhruva"):
            config.path = Path("gs://dhruva-production/dhruva.dhruva")
        return config
    ...
    def _configure_azure_storage(self, config: StorageConfig) -> StorageConfig:
        ...
        if not config.path or config.path == Path("/data/dhruva/dhruva.dhruva"):
            config.path = Path("azure://dhruva-production/dhruva.dhruva")
        return config
