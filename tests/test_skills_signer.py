...
# SignerFeedState lives in dhara.mcp (not in the skills_signer package
...
from dhara.mcp.signer_feed import SignerFeedState
...
class TestCanonicalPayloadForSigning:
    def test_strips_signature_field_from_dict(self) -> None:
        ...
        assert canonical == canonicalize_payload({"name": "dhara-search", "version": "1.0.0"})
    ...
    def test_strips_signature_field_from_pydantic_model(self) -> None:
        ...
        assert b"signature" not in canonical
        assert b"server_pubkey_id" not in canonical
        assert canonical == b'{"name":"dhara-search"}'
...
class TestSigningRoundtrip:
    def test_sign_then_verify_succeeds(self) -> None:
        ...
        metadata = {"name": "dhara-search", "version": "1.0.0", "tool_refs": []}
        canonical = canonicalize_payload(metadata)
        signed = signer.sign(canonical)
    ...
    def test_tampered_payload_fails_verify(self) -> None:
        ...
        canonical = canonicalize_payload({"name": "dhara-search", "version": "1.0.0"})
        signed = signer.sign(canonical)
...
class TestEndToEndFlow:
    def test_server_signs_client_verifies(self) -> None:
        ...
        server_metadata = {
            "name": "dhara-search-insights",
            "version": "1.0.0",
            "description": "Search across Dhara-indexed systems",
            "tool_refs": ["mcp__dhara__search_all_systems"],
            "dependencies": [],
        }
