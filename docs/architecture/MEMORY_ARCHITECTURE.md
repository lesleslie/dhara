...
| `dhara_audit_log` table | `dhara/migrations/sql/0001_initial.sql:41` and `dhara/events/subscribers/audit_log_subscriber.py` | `AuditLogSubscriber` exists but writes nowhere — there is no wiring to commit into the SQL table | `tests/unit/events/test_event_bus.py::test_audit_log_subscriber_writes_row`; track Workstream D |
