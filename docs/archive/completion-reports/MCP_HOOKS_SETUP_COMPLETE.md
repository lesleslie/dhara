...
dhara         :8683  ✅ healthy (67.2% CPU, 602.3 MB RAM)
...
⏸️  dhara      - Persistent storage (leave running)
...
cat ~/.claude/state/mcp-health.json | jq '.servers[] | select(.name == "dhara")'
...
lsof -i :8683  # Check dhara

# Restart a stopped server
cd /Users/les/Projects/dhara && .venv/bin/python -m dhara.cli start --force

# View detailed health logs
cat ~/.claude/logs/mcp-health.log | grep -A 5 "dhara"
```
