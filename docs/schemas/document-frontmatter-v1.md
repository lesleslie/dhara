...
- `.claude/decisions/README.md` (in `mahavishnu/.claude/decisions/`) — the decision index; Status column re-derived from per-decision frontmatter in Phase P5. (Dhara has no `.claude/decisions/` directory of its own; the schema is vendored from mahavishnu.)
