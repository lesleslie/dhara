...
from dhara.core.persistent import GHOST, call_if_persistent
...
class ObjectWriter:
    ...
    def _setup_pickler(self):
        self.sio = BytesIO()
        self.pickler = _PersistentPickler(
            self.sio, PICKLE_PROTOCOL, self._persistent_id
        )
...
class ObjectReader:
    ...
    def _get_unpickler(self, file):
        ...
        # performance boost
        load = functools.partial(persistent_load, self.connection, cache.objects)
