...
class AsyncAdapterRegistry:
    """Async Dhara-backed adapter registry using AsyncConnection."""
    ...
    async def _ensure_root_async(self) -> None:
        ...
        if not is_readonly:
            if "adapters" not in root:
                ...
                changed = True
            if "health_checks" not in root:
                root["health_checks"] = PersistentDict()
                changed = True
    ...
    async def store_adapter_async(
        ...
        """Store or update an adapter in the registry (async version)."""
        await self._ensure_root_async()
        adapters = await self._adapters()
        adapter_id = f"{domain}:{key}:{provider}"
    ...
    async def get_adapter_async(
        ...
        """Retrieve an adapter from the registry (async version)."""
        await self._ensure_root_async()
        adapters = await self._adapters()
    ...
    async def list_adapters_async(
        ...
        """List all adapters with optional filtering (async version)."""
        await self._ensure_root_async()
        root = await self.connection.get_root()
    ...
    async def list_adapter_versions_async(
        ...
        """List all versions of an adapter including rollback history (async version)."""
        await self._ensure_root_async()
        adapters = await self._adapters()
        adapter_id = f"{domain}:{key}:{provider}"
    ...
    async def validate_adapter_async(
        ...
        """Validate an adapter configuration (async version)."""
        adapter = await self.get_adapter_async(domain, key, provider, version)
    ...
    async def check_adapter_health_async(
        ...
        """Check health status of an adapter (async version)."""
        await self._ensure_root_async()
        adapters = await self._adapters()
        health_checks = await self._health_checks()
        adapter_id = f"{domain}:{key}:{provider}"
    ...
    async def count_async(self) -> int:
        """Count total adapters in registry (async version)."""
        await self._ensure_root_async()
        adapters = await self._adapters()
        return len(adapters)
...
async def store_adapter_async_impl(
    ...
    """Async implementation for store_adapter tool."""
    try:
...
async def get_adapter_async_impl(
    registry: AsyncAdapterRegistry,
    domain: str,
    key: str,
    provider: str | None = None,
    ...
    """Async implementation for get_adapter tool."""
    try:
...
async def list_adapters_async_impl(
    registry: AsyncAdapterRegistry,
    domain: str | None = None,
    category: str | None = None,
) -> dict[str, Any]:
    """Async implementation for list_adapters tool."""
    try:
...
async def list_adapter_versions_async_impl(
    registry: AsyncAdapterRegistry,
    domain: str,
    key: str,
    provider: str,
) -> dict[str, Any]:
    """Async implementation for list_adapter_versions tool."""
    try:
...
async def validate_adapter_async_impl(
    registry: AsyncAdapterRegistry,
    domain: str,
    key: str,
    provider: str,
    ...
    """Async implementation for validate_adapter tool."""
    try:
...
async def get_adapter_health_async_impl(
    registry: AsyncAdapterRegistry,
    domain: str,
    key: str,
    provider: str,
) -> dict[str, Any]:
    """Async implementation for get_adapter_health tool."""
    try:
