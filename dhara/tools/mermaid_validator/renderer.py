...
def _locate_mermaid_core() -> Path | None:
    ...
    1. ``DHARA_MERMAID_CORE`` env var (operator-pinned). Must pass
    ...
    env_override = os.environ.get("DHARA_MERMAID_CORE")
    if env_override:
        path = Path(env_override).resolve()
        if _is_trusted_mermaid_path(path):
            return path
        raise RuntimeError(
            f"DHARA_MERMAID_CORE={env_override} is not under a "
...
def _locate_jsdom() -> Path | None:
    ...
    Looks for the `node_modules/jsdom/` directory under the dhara
    ...
    Override via ``DHARA_JSDOM`` env var for CI where the
    dhara repo lives at a different path.
    """
    env_override = os.environ.get("DHARA_JSDOM")
    if env_override:
        path = Path(env_override).resolve()
        if path.is_file():
            return path
        raise RuntimeError(
            f"DHARA_JSDOM={env_override} does not exist or is not a file"
        )

    # Walk up from this file to find the dhara repo root.
...
def _resolve_validator_runtime() -> tuple[Path, Path, Path]:
    ...
    if not jsdom:
        raise RuntimeError(
            ...
            "run `npm install` in the dhara repo to install the "
...
def print_errors(errors: list[MermaidValidationError]) -> None:
    """Pretty-print the broken blocks for `dhara docs check-mermaid`."""
    if not errors:
