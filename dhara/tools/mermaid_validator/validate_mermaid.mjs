...
//
// Designed for the dhara CI guard (test_mermaid_renders.py +
// `dhara docs check-mermaid`). Uses mermaid.parse() — the lexer-only
...
//
// Run:
//   node validate_mermaid.mjs <path-to-mermaid-core.mjs> [<path-to-jsdom>]
//
// The Python wrapper (dhara.tools.mermaid_validator.renderer) computes both
...
if (!mermaidPath) {
  console.error(
    "usage: node validate_mermaid.mjs <path-to-mermaid-core.mjs> [<path-to-jsdom>]",
  );
  process.exit(2);
}
if (!jsdomPath) {
