...
class StorageConfig:
    ...
    def __post_init__(self):
        ...
        if self.backend in ["file", "sqlite"] and not self.path:
            raise ValueError(f"{self.backend.capitalize()} storage requires 'path' to be specified")
...
class DurusConfig:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "DurusConfig":
        ...
        if "path" in storage_data and isinstance(storage_data["path"], (str, Path)):
            ...
            storage_data["path"] = Path(path_val) if isinstance(path_val, str) else path_val
