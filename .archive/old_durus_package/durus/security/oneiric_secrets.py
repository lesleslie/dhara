...
class OneiricSecretsAdapter:
    ...
    def _get_or_create_key(self, key_name: str) -> SecretKey:
        ...
        try:
            ...
            key = SecretKey(
                ...
                expires_at=datetime.fromisoformat(expires_at_str) if expires_at_str else datetime.utcnow() + self.rotation_interval,
                rotation_interval=self.rotation_interval
            )
        ...
        with self._lock:
            for key in self._active_keys.values():
                if len(key.key_material) < 32:  # 256-bit minimum
                    raise ValueError(f"Key {key.key_id} is too short: {len(key.key_material)} bytes")
    ...
    def get_key_status(self) -> Dict[str, Union[str, int, bool]]:
        """
...
_adapter: Optional[OneiricSecretsAdapter] = None
_adapter_lock = threading.Lock()


def get_secrets_adapter(secret_prefix: str = "durus/hmac", rotation_interval: int = 90) -> OneiricSecretsAdapter:
    """
...
def verify_hmac_signature(message: bytes, signature: bytes, algorithm: str = "sha256") -> bool:
    """
...
def initialize_secrets(secret_prefix: str = "durus/hmac", rotation_interval: int = 90) -> None:
    """
