...
class BackupCatalog:
    ...
    def get_last_backup_of_type(self, backup_type: BackupType) -> Optional[BackupMetadata]:
    ...
    def get_differential_backups(self, base_backup_id: str) -> List[BackupMetadata]:
        ...
        return [
            b for b in self.get_all_backups()
    ...
    def search_backups(
        ...
        if contains_string:
            results = [b for b in results if contains_string.lower() in b.backup_id.lower()]
    ...
    def get_backup_statistics(self) -> Dict[str, Any]:
        ...
        retention_compliance = (compliant_backups / len(backups)) * 100 if backups else 0
        ...
        for backup in self.get_all_backups():
            ...
            if current_time > retention_date:
                ...
                if self.remove_backup(backup.backup_id):
                    removed_count += 1
                    logger.info(f"Removed expired backup from catalog: {backup.backup_id}")
        ...
        for backup in self.get_all_backups():
            if backup.backup_type in [BackupType.INCREMENTAL, BackupType.DIFFERENTIAL]:
                if not backup.parent_backup_id:
                    ...
                    if not parent:
                        issues.append(f"Missing parent backup: {backup.backup_id} depends on {backup.parent_backup_id}")

        # Check for missing backup files
        for backup in self.get_all_backups():
            backup_path = Path(backup.source_path)
            if not backup_path.exists():
                issues.append(f"Missing backup file: {backup.backup_id} at {backup.source_path}")
