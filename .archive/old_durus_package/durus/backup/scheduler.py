        ...
        try:
            ...
            if self.backup_manager.cloud_adapter:
                ...
                if not cloud_success:
                    logger.warning(f"Cloud upload failed for backup: {backup_metadata.backup_id}")
...
class BackupScheduler:
    ...
    def __init__(
        ...
        self.verification_engine = BackupVerification(backup_dir) if backup_dir else None
    ...
    def run_job(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        while self.running:
            try:
                ...
                if (self.last_verification is None or
                    (now - self.last_verification).seconds >= self.verify_interval):
                    ...
                    for check_name, result in results.items():
                        if result["status"] == "failed":
                            logger.error(f"Verification failed: {check_name} - {result.get('error', 'Unknown error')}")
                        else:
