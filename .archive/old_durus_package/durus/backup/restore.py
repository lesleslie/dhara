...
class RestoreManager:
    ...
    def __init__(
        ...
        self.encryption = EncryptionEngine(key=encryption_key) if encryption_key else None
    ...
    def _restore_from_backup(self, backup_metadata: BackupMetadata) -> str:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not backup_path.exists() and self.cloud_adapter:
                self.logger.info(f"Downloading backup from cloud: {backup_metadata.backup_id}")
            ...
            self.logger.info(f"Database restored from backup: {backup_metadata.backup_id}")
    ...
    def restore_point_in_time(
        ...
        if not restore_points:
            raise ValueError(f"No backup available for point-in-time restore to {target_time}")

        # Select the most appropriate backup
        if use_incremental:
            # Try to use incremental backup if available
            incremental_points = [rp for rp in restore_points if rp.restore_type == "incremental"]
            if incremental_points:
                backup = BackupCatalog(self.backup_dir).get_backup(incremental_points[0].backup_id)
            else:
                backup = BackupCatalog(self.backup_dir).get_backup(restore_points[0].backup_id)
        else:
    ...
    def restore_incremental_chain(self, base_backup_id: str) -> str:
        ...
        self.logger.info(f"Starting incremental restore from base backup: {base_backup_id}")
        ...
        for incremental_backup in incremental_backups:
            self.logger.info(f"Applying incremental backup: {incremental_backup.backup_id}")
    ...
    def verify_restore(self, backup_metadata: BackupMetadata) -> bool:
        ...
        try:
            ...
            if os.path.getsize(self.target_path) != backup_metadata.size_bytes:
                self.logger.error(f"File size mismatch: expected {backup_metadata.size_bytes}, got {os.path.getsize(self.target_path)}")
                return False

            # Check checksum
            checksum = self._calculate_checksum(self.target_path)
            if checksum != backup_metadata.checksum:
                self.logger.error(f"Checksum mismatch: expected {backup_metadata.checksum}, got {checksum}")
                return False
    ...
    def get_restore_summary(self) -> Dict[str, Any]:
        ...
        by_type = {
            ...
            "incremental": [b for b in backups if b.backup_type == BackupType.INCREMENTAL],
            "differential": [b for b in backups if b.backup_type == BackupType.DIFFERENTIAL]
        }
