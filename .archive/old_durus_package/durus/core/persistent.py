...
try:
    ...
    def _hasattribute(obj: Any, name: str) -> bool:
        try:
...
class PersistentObject(PersistentBase):
    ...
    def _p_set_status_ghost(self) -> None:
        self.__setstate__({})
        _setattribute(self, "_p_status", GHOST)

    def _p_set_status_saved(self) -> None:
        _setattribute(self, "_p_status", SAVED)

    def _p_set_status_unsaved(self) -> None:
        if self._p_status == GHOST:
            self._p_load_state()
...
class Persistent(PersistentObject):
    ...
    def __getstate__(self) -> dict[str, Any]:
        return _getattribute(self, "__dict__")
